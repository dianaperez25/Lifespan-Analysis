function seg_index_reliability(subject, neg_corrs, data_dir, parcellation, output_str, iterations)
...
data_dir = sprintf('/scratch/dcr8536/%s_Parcellation/', parcellation);
...
for s = 1:numel(subject)  
            ...
            mat_struct = load(sprintf('%s/avg_timecourses/corrmats/sub-%s_indiv_parcels_corrmats_sorted.mat', data_dir, subject{s}));
           catData = mat_struct.sorted_ts;
           parcel_netid = mat_struct.parcel_netid;
            %clear sorted_corrmat sorted_ts

    %I think this should be 10,816
    disp(sprintf('Total number of sample points for subject %s is %d...', subject{s}, size(catData,2)))
   ...
   count = datasample(1:num_sets,1);
    ...
    [seg_index_truehalf(p,s)] = calculate_seg_index(corrmat_truehalf, parcel_netid);
    ...
    for t = 1:numel(times)
        ...
        [sub_seg_index(p,t) between_FC(p,t) within_FC(p,t)] = calculate_seg_index(corrmat, parcel_netid);
    end
...
ax.FontSize = 24;
% 
print(gcf,[output_dir '/KongReliabilityLifespanSegIndex_Diff_' output_str '.jpg'],'-dpng','-r300');
...
function [seg_index between_FC within_FC] = calculate_seg_index(matrix, parcel_netid)
all_within = [];
all_between = [];
matrix = single(FisherTransform(matrix));% fisher transform r values
networks = unique(parcel_netid(:,2));
num_rois = length(parcel_netid);
count = 1;
for net = 1:length(networks)
    net_size = length(find(parcel_netid(:,2)==networks(net)));
    if net == 1
        rois = [count:net_size];
    else
        rois = [count:(count-1) + net_size]; %extract the rois belonging to system n
    end
