function post_fc_processing_batch_GrattonLab(post_fc_processing_batch_params_file, datalist)
...
workbenchdir = ['/projects/b1081/Scripts/work' ...
    'bench2/bin_linux64/'];
...
for i=1:numdatas
    ...
    for r = 1:length(run_nums{i})
        allstart_runs_fstring2{i,r} = sprintf('sub-%s_ses-%d_task-%s_run-%d',subjectID{i},dataInfo.sess(i),dataInfo.task{i},run_nums{i}(r));
...
for s = 1:length(dataInfo.sub)
    ...
    for r = 1:length(run_nums{s})
        ...
        if ~exist([subfunc_run '.nii.gz']) && ~exist([subfunc_run '.nii'])
        ...
        %system(['module load fsl/5.0.8; fslmaths ' subfunc_run ' -nan ' subfunc_run_this]); %AD commented out 3/2023
        system(['fslmaths ' subfunc_run ' -nan ' subfunc_run_this]);
...
%for s = 1:length(dataInfo.sub)
    ...
    if ~exist(preproc_concat_data_outname) || force_make_concat
        ...
        for r = 1:length(run_nums{s})
            ...
            data = load_nii(preprocdata_names{s,r});
        ...
        save_nii(out,preproc_concat_data_outname);
    else
