...
%%TO-DO:
%ADD SOME LINES TO CALCULATE A DIFF BETWEEN TRUE HALF AND IND SAMPLE TO
%1. SEE IF THERE ARE SPECIFIC NETS DRIVING THE DIFFS (CHECK STEPHANIE NOBLE
%AND MAXWELL SOMETHING?? FOR THIS TYPE OF ANALYSIS BREAKING IT DOWN BY
%NETWORK)
%2. THE DIFFS ARE SYSTEMATIC/PREDICTABLE?
clear all
...
atlas_params = atlas_parameters_GrattonLab('Parcels333','/Volumes/fsmresfiles/PBS/Gratton_Lab/Atlases/');
...
for sub = 1:numel(subject)    
    ...
    for p = 1:iterations
        ...
        corrmat_truehalf = FisherTransform(paircorr_mod(truehalf'));
        ...
        for t = 1:numel(times) % for each of the time steps that we calculated before...
            ...
            corrmat = FisherTransform(paircorr_mod(sampledDatas{t}'));
            ...
            diffs_matrix = corrmat_truehalf - corrmat;
            figure;
            imagesc(diffs_matrix, [-1 1])
            colormap(better_jet_colormap_diff);
            saveas(gcf,['/Users/dianaperez/Desktop/reliability_diffs/sub-' subject{sub} '_diff_step-' num2str(t) '.tiff'], 'tiff')
        end
