...
%% OPTIONS
datasets = {'Lifespan-NU', 'iNet-NU', 'Lifespan-FSU', 'iNet-FSU'}; %'Lifespan-NU', 
...
for sub = 1:numel(subject)    
    ...
    disp(sprintf('data loading and storage finished -- time elapsed: %02d seconds', toc(t0)/60))
    ...
    for p = 1:iterations
        ...
        disp(sprintf('data concatenation finished: time elapsed %02d minutes (%02d seconds since start of iteration)', toc(t0)/60, toc(t1)))
