%% Stability of template-match network maps
clear all

%---------------------------------------------------------------------------
subjects = {'LS02', 'LS03', 'LS04', 'LS05', 'LS07', 'LS08', 'LS10', 'LS11', 'LS14'};
datalist_loc = %path to txt file
cifti_dir = ' ';
out_dir = ' ';


for sub = 1:numel(subjects)
    sub_netmap = ['sub-' subjects{sub} '_dice_WTA_map_kden0.05.dtseries.nii'];
    if ~exist(sub_netmap, 'file') %% I'm going to have to make two template match maps,
        % but how am I going to divide the data? 
        [net_map1, net_map2] = Template_matching_cortex_WU120_test-retest(subjects, datalist_loc, cifti_dir, out_dir);
    end
    % once I have both network maps, will probably have to test the
    % correlation between the two. So the output of this script will
    % probably end up being a similarity matrix showing the correlation
    % values for both----right? 
    %- do we have a minimum amount of data that we should do this for and expect accurate results?
    
end


% Will probably have to
% modify the template matching script to concatenate all the data,
% and then only do the template matching based on each half
% separately
function [net_map1 net_map2] = Template_matching_cortex_WU120_test-retest(subjects, datafiles, cifti_dir, outDir)
...
for s=1:length(subjects)
    ...
    for t=1:length(IDs)        
        ...
        dice_to_templates(t,:)= (sum((big_cifti_template_mat_full & corr_mat_thresh),2)*2) ./ sum((big_cifti_template_mat_full | corr_mat_thresh),2);
    end
